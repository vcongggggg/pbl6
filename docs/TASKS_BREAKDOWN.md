...
| **`TASK-4.3`** | `#21` | **Tiền xử lý & Phân chia Stratified Split Dual-Track:** Làm sạch và phân tầng 70/15/15 theo kiến trúc 2 nhánh: Track 1 thuần 4 lớp cho Model Tấn Công (`data/processed/attack/`), Track 2 đa lớp 5 lớp cho Model Phòng Thủ (`data/processed/defense/`). | **Thành viên A** | **HOÀN THÀNH ✅** | `data/processed/attack/`, `data/processed/defense/` |
...
| **`TASK-5.3`** | `#24` | **Đóng Gói Champion Model Artifact, Metadata & Kiểm Thử Gateway:** Đóng gói mô hình vô địch `rf_model.joblib` kèm `rf_metadata.json` (schema 17 features, version, thresholds, metrics) cho Gateway nạp nóng; kiểm thử tích hợp Gateway WAF đảm bảo sub-15ms latency budget. | **Thành viên A** | **SẴN SÀNG TẠO PR 🚀** | `ml-engine/artifacts/rf_model.joblib`, `rf_metadata.json` |
...
| Mã Task | Issue ID | Tên Task Chi Tiết | Phụ Trách | Trạng Thái | Đầu Ra (Deliverables) |
| :--- | :---: | :--- | :---: | :---: | :--- |
| **`TASK-9.1`** | `#35` | **Xây Dựng REST APIs Thống Kê Dashboard (NIST SP 800-137, ISO/IEC 27004):** Các endpoint `GET /api/dashboard/stats`, `/events`, `/timeline`, `/distribution`, `/simulate`, `/reset-demo`, `/seed-demo`, `/toggle-waf-mode` trên FastAPI Gateway với chuẩn hóa học thuật và telemetry metrics. | **Thành viên A** | **HOÀN THÀNH ✅** | `gateway/app/api/dashboard.py`, `docs/reports/phase9_dashboard_telemetry_academic.md` |
| **`TASK-9.2`** | `#36` | **Thẻ Chỉ Số Tổng Quan & Biểu Đồ Hoạt Động Thời Gian Thực (NIST SP 800-137, ISO/IEC 27004, IEEE TNSM 2021):** Hệ thống 5 KPI Cards (Total Traffic, Attacks Detected, Dual Threat & Risk Scoring, Safe Rate, Quick Simulator) và biểu đồ sóng đôi Threat Timeline Area Chart + Donut Chart. | **Thành viên A** | **HOÀN THÀNH ✅** | `dashboard/src/components/MetricCards.tsx`, `ThreatTimelineChart.tsx`, `AttackDistributionChart.tsx`, `docs/reports/phase9_kpi_charts_academic.md` |
| **`TASK-9.3`** | `#37` | **Bảng Nhật Ký Sự Kiện Tấn Công & Bộ Thanh Tra Payload (NIST SP 800-92, NIST SP 800-86):** Bảng hiển thị sự kiện thời gian thực (Live Events Table) 9 cột viễn trắc, nhận diện IP Origin (LAN Máy 2 vs Localhost), tìm kiếm mờ, lọc đa chiều, và Drawer đối sánh Raw vs Canonical input kèm Vector 17 đặc trưng học máy XAI. | **Thành viên A** | **HOÀN THÀNH ✅** | `dashboard/src/components/events/LiveEventsTable.tsx`, `PayloadEvidenceDrawer.tsx`, `docs/reports/phase9_events_inspector_academic.md` |
| **`TASK-9.4`** | `#38` | **Hộp Thoại Giải Thích Quyết Định WAF & XAI Feature Attribution (NIST SP 800-137, ISO/IEC 27004, ACM CSUR):** Trực quan hóa tỷ trọng đóng góp rủi ro đa tầng Rule (40%) + Supervised ML (35% - Model-Agnostic XGBoost Champion 🏆 / RF Fallback) + Isolation Forest Anomaly (25%), hiển thị ma trận quyết định 4 mức, phân tích bối cảnh CWE/CAPEC/MITRE và xuất báo cáo JSON 1-click. | **Thành viên A** | **HOÀN THÀNH ✅** | `dashboard/src/components/explain/DetectionExplainabilityModal.tsx`, `docs/reports/phase9_explainability_modal_academic.md` |
| **`TASK-9.5`** | `#63` | **Hệ Thống Bắn Thử Nghiệm Tấn Công & Điều Khiển Diễn Tập An Ninh Trực Tiếp (NIST SP 800-115, ISO/IEC 27004):** Bảng Quick Attack Simulator 5 véc-tơ (SQLi, XSS, Path, Cmd, Benign) qua in-memory ASGITransport, WAF Mode Switcher tức thời (Monitor vs Active Blocking), nút Reset Demo Data và Seed Demo Data phục vụ diễn tập trực tiếp trước Hội đồng. | **Thành viên A** | **HOÀN THÀNH ✅** | `dashboard/src/components/MetricCards.tsx`, `dashboard/src/components/Header.tsx`, `gateway/app/api/dashboard.py`, `docs/reports/phase9_demo_simulator_academic.md` |
