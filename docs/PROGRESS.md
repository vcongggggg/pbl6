...
### Phase 5 — Tasks 5.1 - 5.3: Multi-Model Benchmarking & Random Forest Training Pipeline (COMPLETED 100% ✅)
...
* **Mục tiêu (Objectives):**
  * Xây dựng pipeline huấn luyện và đối sánh đồng thời **5 mô hình ứng viên** đại diện 5 trường phái thuật toán máy học trên tập dữ liệu 20.000 mẫu:
    1. `Logistic Regression (Multinomial)`: Baseline tuyến tính.
    2. `Decision Tree (CART)`: Cây quyết định phi tuyến đơn lẻ.
  ...
  * Chọn lựa Mô hình vô địch (**Random Forest**) xuất artifact `ml-engine/artifacts/rf_model.joblib` và `rf_metadata.json`.
  * Cung cấp Jupyter Notebook `ml-engine/notebooks/01_train_and_benchmark.ipynb` trực quan hóa Confusion Matrix heatmap và Feature Importance phục vụ trình diễn bảo vệ đồ án.

* **Sản phẩm bàn giao (Deliverables):**
  * `ml-engine/models/train_rf.py` & `ml-engine/models/evaluate.py`: Pipeline huấn luyện, đối sánh đa mô hình và tối ưu siêu tham số.
  * `docs/reports/rf_evaluation.md`: Báo cáo đối chiếu số liệu thực nghiệm 5 mô hình, bảng ma trận nhầm lẫn và phân tích tầm quan trọng đặc trưng.
  * `ml-engine/artifacts/rf_model.joblib` & `rf_metadata.json`: Model vô địch đóng gói nạp vào Gateway.
  * `ml-engine/notebooks/01_train_and_benchmark.ipynb`: Notebook trực quan hóa phục vụ bảo vệ.
...
* **Kiểm thử & Xác minh (Tests & Verification):**
  ...
  * **Champion Random Forest Results (Test Set N=3,000):**
    * F1-Score (Macro): **99.93%**
    * False Positive Rate (FPR): **0.00%** (0 / 1,500 Benign false alarms)
    * Youden's Index $J$: **0.9989** ($\ge 0.90$ threshold)
    * Single-Sample Predict Latency: **< 10ms** (well within the $\le 15.0\text{ms}$ budget)
